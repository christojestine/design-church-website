...
- Fonts: Noto Sans Malayalam sits behind DM Sans and Rachana (SMC, from smc.org.in; Noto Serif Malayalam as fallback) behind Cinzel in every font stack; Google Fonts only downloads them when Malayalam is on screen. `theme.css` resets letter spacing and raises heading line height under `html:lang(ml)`.
